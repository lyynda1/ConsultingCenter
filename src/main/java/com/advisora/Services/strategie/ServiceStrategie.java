...
public class ServiceStrategie implements IService<Strategie> {
    ...
    public void ajouter(Strategie strategie) {
        ...
        try (Connection cnx = MyConnection.getInstance().getConnection();
            ...
            if (!Boolean.TRUE.equals(strategie.getApprobation())) {
                ps.setString(1, resolveDbStrategyStatus(cnx, StrategyStatut.EN_ATTENTE));
            } else if (strategie.getProjet() == null) {
                ps.setString(1, resolveDbStrategyStatus(cnx, StrategyStatut.NON_AFFECTEE));
            } else {
                ps.setString(1, resolveDbStrategyStatus(cnx, StrategyStatut.EN_COURS));
            }
            ...
            // Ã¢Å“â€¦ DureeTerme is INT in DB + model
    ...
    public void modifier(Strategie strategie) {
        ...
        try (Connection cnx = MyConnection.getInstance().getConnection();
            ...
            // 4 idUser Ã¢Å“â€¦ FIXED (was wrong in your old code)
    ...
    public void updateProject(Strategie s, UserRole role) {
        ...
        if (ref == null) {
            throw new IllegalStateException("Changement projet bloquÃƒÂ©: lockedAt inconnue.");
        }

        long minutes = java.time.Duration.between(ref, LocalDateTime.now()).toMinutes();
        if (minutes > 120) {
            throw new IllegalArgumentException("Vous ne pouvez plus changer le projet aprÃƒÂ¨s 2 heures. Seul l'admin peut.");
        }
    ...
    public void applyDecision(int idStrategie, boolean accepted, String justificationIfRefused, boolean detachOnRefuse) {
        ...
        if (!accepted) {
            if (justificationIfRefused == null || justificationIfRefused.trim().isEmpty()) {
                throw new IllegalArgumentException("Le refus nÃƒÂ©cessite une justification.");
            }
        }
        ...
        try (Connection cnx = MyConnection.getInstance().getConnection();
            ...
            throw new RuntimeException("Erreur mise ÃƒÂ  jour dÃƒÂ©cision strategie: " + e.getMessage(), e);
        }

        NotificationManager.getInstance().addNotification(new Notification(
                "DÃƒÂ©cision strategie",
                "La strategie a ÃƒÂ©tÃƒÂ© " + (accepted ? "acceptÃƒÂ©e" : "refusÃƒÂ©e")
        ));
    }
    ...
    public Strategie recommendDraftStrategy(Project project) {
        ...
        if (sim.isDuplicate()) {
            int matchId = sim.getBestMatchingStrategyId(); // Ã¢Å“â€¦ based on your SimilarityResult class
    ...
    public Strategie recommendStrategy(Project project) {
        ...
        if (result.isDuplicate()) {
            int matchId = result.getBestMatchingStrategyId(); // Ã¢Å“â€¦ match SimilarityResult
    ...
    public Strategie confirmRecommendedStrategy(Strategie draft) {
        if (draft == null) throw new IllegalArgumentException("StratÃƒÂ©gie invalide.");
        ...
        if (draft.getDureeTerme() <= 0) {
            throw new IllegalArgumentException("DurÃƒÂ©e du terme doit ÃƒÂªtre > 0 (en jours).");
        }
    ...
    public int getBestMatchingStrategyId(Project project) {
        ...
        return result.getBestMatchingStrategyId(); // Ã¢Å“â€¦
    }
...
""".formatted(
    ...
    private void validate(Strategie s, boolean create) {
        ...
        if (s.getBudgetTotal() > 1_000_000_000)
            throw new IllegalArgumentException("Budget total trop ÃƒÂ©levÃƒÂ© (max 1 milliard).");

        if (s.getGainEstime() < 0) throw new IllegalArgumentException("Gain estimÃƒÂ© >= 0");
        if (s.getGainEstime() > 1_000_000_000)
            throw new IllegalArgumentException("Gain estimÃƒÂ© trop ÃƒÂ©levÃƒÂ©.");
