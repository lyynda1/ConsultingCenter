...
public class ExchangeRateController {
    ...
    private void showLoading() {
        statusLabel.setText("â³ Chargement des taux en cours...");
    ...
    private void showRates(Map<String, Double> rates) {
        ...
        usdLabel.setText(rates.containsKey("USD") ? String.format("%.4f", rates.get("USD")) : "â€”");
        eurLabel.setText(rates.containsKey("EUR") ? String.format("%.4f", rates.get("EUR")) : "â€”");
        gbpLabel.setText(rates.containsKey("GBP") ? String.format("%.4f", rates.get("GBP")) : "â€”");

        ratesGrid.setVisible(true);
        ratesGrid.setManaged(true);

        noteLabel.setText("Taux mis Ã  jour en temps rÃ©el via ExchangeRate-API");
        noteLabel.setVisible(true);
        noteLabel.setManaged(true);
    }

    private void showError(Throwable ex) {
        statusLabel.setText("âŒ " + (ex != null ? ex.getMessage() : "Erreur inconnue"));
