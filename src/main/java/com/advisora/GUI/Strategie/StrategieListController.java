...
public class StrategieListController {
    ...
    private static LocalDateTime computeDeadlineFromLockedAt(Strategie s) {
        ...
        int days = s.getDureeTerme();     // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ int (DB int)
        if (days <= 0) return null;

        LocalDateTime lockedAt = s.getLockedAt(); // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ LocalDateTime
        if (lockedAt == null) return null;
    ...
    private static String formatRemaining(long sec) {
        if (sec <= 0) return "ExpirÃƒÆ’Ã‚Â©e";
        long days = sec / 86400;
    ...
    public void initialize() {
        ...
        if (cmbStatusFilter != null) {
            cmbStatusFilter.setItems(FXCollections.observableArrayList(
                    ...
                    StrategyStatut.EN_ATTENTE,
                    StrategyStatut.ACCEPTEE,
                    StrategyStatut.REFUSEE,
                    StrategyStatut.NON_AFFECTEE
            ));
        ...
        strategieList.setCellFactory(lv -> new ListCell<>() {
            ...
            /** One timer per cell ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ keeps the countdown ticking. */
            ...
            protected void updateItem(Strategie s, boolean empty) {
                ...
                // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ 1) stop old timer because ListView cells are reused
                if (countdownTimeline != null) {
                    countdownTimeline.stop();
                    countdownTimeline = null;
                }

                // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ 2) empty cell
                if (empty || s == null) {
                    setGraphic(null);
                    return;
                }

                // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ 3) build UI card + get the countdown label directly (no lookup)
                ...
                // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ 4) compute deadline using LOCKED_AT (start) + dureeTerme (days)
                ...
                // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ 5) tick function
                Runnable tick = () -> {
                    long secsLeft = remainingSeconds(deadline);

                    if (secsLeft <= 0) {
                        countdownLbl.setText("DurÃƒÆ’Ã‚Â©e restante : ExpirÃƒÆ’Ã‚Â©e");
                        ...
                        countdownLbl.setText("DurÃƒÆ’Ã‚Â©e restante : " + formatRemaining(secsLeft));
                        // optional styling class
                ...
                // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ 6) start timeline (updates every second)
            ...
            /* -----------------------------------------------------------------------
             * Helpers ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ keep them in the controller so all other methods can use them
             * ----------------------------------------------------------------------- */
    ...
    public void onRefresh(ActionEvent e) {
        try {
            ...
            ex.printStackTrace();
            showError("RafraÃƒÆ’Ã‚Â®chissement impossible: " + ex.getMessage());
        }
    }
    ...
    private void refresh() throws Exception {
        ...
        // project filter items (keep selection)
        if (cmbProjectFilter != null) {
            ...
            if (current != null && items.contains(current)) cmbProjectFilter.setValue(current);
            else cmbProjectFilter.setValue("Tous");
        }
    ...
    private CardWithCountdown buildCard(Strategie s) {
        ...
        String statusIcon = (s.getStatut() == null) ? "" : switch (s.getStatut()) {
            case ACCEPTEE -> "ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ ";
            case REFUSEE -> "ÃƒÂ¢Ã¢â‚¬ÂºÃ¢â‚¬Â ";
            case EN_COURS -> "ÃƒÂ¢Ã‚ÂÃ‚Â³ ";
            case NON_AFFECTEE -> "ÃƒÂ¢Ã…Â¡Ã‚Âª ";
            case EN_ATTENTE -> "ÃƒÂ°Ã…Â¸Ã¢â‚¬Â¢Ã¢â‚¬â„¢ ";
            default -> "";
        };
        ...
        Label projet = new Label("Projet associÃƒÆ’Ã‚Â© : " +
                (s.getProjet() == null ? "-" : safe(s.getProjet().getTitleProj())));
        projet.getStyleClass().add("card-meta");

        Label date = new Label("Date de crÃƒÆ’Ã‚Â©ation : " +
        ...
        // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ ALWAYS show dureeTerme (even if null -> "-")
        Integer d = s.getDureeTerme();
        Label duree = new Label("DurÃƒÆ’Ã‚Â©e du terme : " + (d == null ? "-" : d + " jours"));
        duree.getStyleClass().add("card-meta");
        duree.setVisible(true);
        duree.setManaged(true);

        // (Optional) show lockedAt for debugging
        Label lockedAtLbl = new Label("LockedAt : " + (s.getLockedAt() == null ? "-" : s.getLockedAt().toString()));
        lockedAtLbl.getStyleClass().add("card-meta");

        // Countdown label (will be used later, keep hidden for now)
        ...
        VBox metaLeft = new VBox(6, projet, date, type, duree, lockedAtLbl, countdown);
        ...
        Label gain = new Label("Gain estimÃƒÆ’Ã‚Â© : " + gainText);
    ...
    private void acceptStrategie(Strategie s) {
        Alert confirm = new Alert(Alert.AlertType.CONFIRMATION,
                "Accepter cette stratÃƒÆ’Ã‚Â©gie ?\nID: " + s.getId(),
    ...
    private void refuseStrategie(Strategie s) {
        ...
        dialog.setHeaderText("Refuser cette stratÃƒÆ’Ã‚Â©gie");
    ...
    private void applyLocalStylesheet() {
        if (pageRoot == null) return;
        URL cssUrl = getClass().getResource("/css/strategie-list.css");
        if (cssUrl == null) return;
        String external = cssUrl.toExternalForm();
        if (!pageRoot.getStylesheets().contains(external)) {
            pageRoot.getStylesheets().add(external);
        }
    }

    // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ THIS is the countdown core used by the cell:
