...
// Set the filtered list to the ComboBox
    ...
    public void setEditingStrategie(Strategie strategie) {
        ...
        terme.setText(String.valueOf(strategie.getDureeTerme()));
        if (titleLabel != null) {
            titleLabel.setText("Modifier StratÃ©gie");
        }
        if (saveBtn != null) {
    ...
    public void resetForm() {
        ...
        gainEstimeField.setText("0");
        if (titleLabel != null) {
            titleLabel.setText("Nouvelle StratÃ©gie");
        }
        if (saveBtn != null) {
    ...
    private void save() {
        ...
        try {
            ...
            s.setDureeTerme(parsePositiveInt(terme.getText(), "DurÃ©e/terme obligatoire."));
        ...
        if (s.getBudgetTotal() > s.getGainEstime()) {
            Alert a = new Alert(Alert.AlertType.CONFIRMATION,
                    "Le gain estimÃ© est infÃ©rieur au budget total. Continuer ?",
        ...
        Task<RiskOutcome> task = new Task<>() {
            ...
            protected RiskOutcome call() throws Exception {
                ...
                String correctedTitle = riskService.applyCorrectionsToTitle(s.getNomStrategie(), rr.suggestions);

// update title used by LLM too
                title = correctedTitle;

// (optional) keep a message showing corrections
                if (rr.suggestions != null && !rr.suggestions.isEmpty()) {
                    improvedMsg += "\n\nCorrections auto:\n- " + String.join("\n- ", rr.suggestions);
                }

// Save corrected title
                s.setNomStrategie(correctedTitle);
            ...
            try {
                switch (finalSeverity) {
                    case INFO, WARNING -> {
                        ...
                        showInfo("EnregistrÃ©e (Risque: " + finalSeverity + ")", improvedMsg);
                        onSaved.run();
                        close();
                    }
                    case DANGER -> {
                        ...
                        serviceStrategie.ajouter(s);
                        showInfo("EnregistrÃ©e avec justification (Risque: DANGER)", improvedMsg);
                        onSaved.run();
                        close();
                    }
                    case CRITICAL -> {
                        ...
                        dialog.setHeaderText("Cette stratÃ©gie prÃ©sente un risque critique.\nUne justification est obligatoire pour continuer.");
                        ...
                        justificationArea.setPromptText("Justification obligatoire (mesures, mitigation, pourquoi la stratÃ©gie reste viable)...");
                        justificationArea.setWrapText(true);
                        justificationArea.setPrefRowCount(6);

                        VBox content = new VBox(10,
                                new Label("DÃ©tails du risque :"),
                                riskDetails,
                        ...
                        // âœ… if you have a field in Strategie, store it:
                        // s.setJustification(justification);

                        serviceStrategie.ajouter(s);

                        // âœ… if you store justification in DB via a service call:
                        // serviceStrategie.updateJustification(s.getId(), justification);

                        NotificationManager.getInstance().addNotification(new com.advisora.Model.strategie.Notification(
                                "Approbation requise (CRITICAL)",
                                "StratÃ©gie: '" + s.getNomStrategie() + "'. Justification: " + justification
                        ));
    ...
    private String validateStrategyName(String name) {
        if (name == null) throw new IllegalArgumentException("Nom stratÃ©gie obligatoire.");
        String n = name.trim();

        if (n.length() < 5)
            throw new IllegalArgumentException("Nom stratÃ©gie trop court (min 5 caractÃ¨res).");

        if (!n.matches("[\\p{L}0-9\\s\\-_'â€™]+"))
            throw new IllegalArgumentException("Nom stratÃ©gie contient des caractÃ¨res invalides.");

        if (n.matches("(?i).*([\\p{L}0-9])\\1{4,}.*")) {
            throw new IllegalArgumentException("Nom stratÃ©gie non valide (trop rÃ©pÃ©titif).");
        }
        ...
        if (lettersOnly.length() >= 6 && distinct <= 2)
            throw new IllegalArgumentException("Nom stratÃ©gie non valide (trop alÃ©atoire).");

        if (!n.toLowerCase().matches(".*[aeiouyÃ Ã¢Ã¤Ã©Ã¨ÃªÃ«Ã®Ã¯Ã´Ã¶Ã¹Ã»Ã¼].*"))
            throw new IllegalArgumentException("Nom stratÃ©gie non valide (doit ressembler Ã  un mot).");

        return n; // âœ… return cleaned valid name
    }
    ...
    private double parsePositiveDouble(String text, String s) {
        try {
            ...
            if (v > 1_000_000_000) {
                throw new IllegalArgumentException(s + " c'est exagÃ©rÃ© veuillez verifier.");
            }
            return v;
