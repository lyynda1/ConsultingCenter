...
public class AddInvestissementDialogController {
    ...
    public void initialize() {
        ...
        if (dureeUnitCombo != null) {
            dureeUnitCombo.getItems().setAll("Heures", "Jours", "Semaines", "Mois", "Années");
    ...
    private void save() {
        ...
        try {
            ...
            new Alert(Alert.AlertType.ERROR, "Durée invalide. Saisissez un nombre.").showAndWait();
            return;
        }
        if (dureeValue <= 0) {
            new Alert(Alert.AlertType.ERROR, "La durée doit être positive.").showAndWait();
            return;
        }
        ...
        switch (unit) {
            ...
            case "Années" -> hours = dureeValue * 365 * 24;
        ...
        if (budMin < 0 || budMax < 0) {
            new Alert(Alert.AlertType.ERROR, "Les budgets ne peuvent pas être négatifs.").showAndWait();
            return;
        }
        if (budMin > budMax) {
            new Alert(Alert.AlertType.ERROR, "Le budget min ne peut pas être supérieur au budget max.").showAndWait();
            return;
        }
