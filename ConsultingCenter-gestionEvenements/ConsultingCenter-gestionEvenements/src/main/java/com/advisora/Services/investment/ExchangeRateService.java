 ...
 * Base currency: TND (Dinar Tunisien – affiché "DNT" dans l'application).
...
public class ExchangeRateService {
    ...
    public Map<String, Double> getRates() throws Exception {
        ...
        if (response.statusCode() != 200) {
            throw new Exception("Erreur HTTP " + response.statusCode() + " lors de l'appel à l'API.");
        }
        ...
        if (!"success".equalsIgnoreCase(result)) {
            ...
            throw new Exception("Réponse API invalide : " + errorType);
        }
