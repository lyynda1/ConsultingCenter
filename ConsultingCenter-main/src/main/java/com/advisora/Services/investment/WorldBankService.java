...
public class WorldBankService {
    ...
    private double[] fetchLatest(String indicator) throws Exception {
        ...
        throw new Exception("Aucune donnÃ©e disponible pour l'indicateur " + indicator);
    }
    ...
    private JsonNode fetchDataArray(String indicator) throws Exception {
        ...
        if (resp.statusCode() != 200) {
            throw new Exception("HTTP " + resp.statusCode() + " â€” " + indicator);
        }
        ...
        if (data == null || data.isNull() || data.size() == 0) {
            throw new Exception("DonnÃ©es manquantes (World Bank) : " + indicator);
        }
        return data;
    }
}
