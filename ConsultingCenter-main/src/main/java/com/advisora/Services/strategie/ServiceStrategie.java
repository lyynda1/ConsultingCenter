...
public class ServiceStrategie implements IService<Strategie> {
    ...
    public List<Strategie> afficher() {
        ...
        try (Connection cnx = MyConnection.getInstance().getConnection();
             PreparedStatement ps = cnx.prepareStatement(buildStrategySelectSql(cnx, resolveStrategyTableName(cnx), "ORDER BY s.idStrategie DESC"));
    ...
    public List<Objective> getObjectivesByStrategie(int strategieId) {
        List<Objective> list = new ArrayList<>();
        String sql = "SELECT * FROM objectives WHERE ids = ?"; // adapte le nom colonne/table
        ...
        try (Connection conn = MyConnection.getInstance().getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    o.setId(rs.getInt("ids"));
                    o.setNomObjective(rs.getString("nomObj"));
                    // ... remplis les autres champs selon ta table
                    list.add(o);
                }
            }
        } catch (SQLException e) {
    ...
    public Strategie getById(int idStrategie) {
        try (Connection cnx = MyConnection.getInstance().getConnection();
             PreparedStatement ps = cnx.prepareStatement(buildStrategySelectSql(cnx, resolveStrategyTableName(cnx), "WHERE s.idStrategie=?"))) {
    ...
    public List<Strategie> getByProject(int projectId) {
        List<Strategie> list = new ArrayList<>();
        try (Connection cnx = MyConnection.getInstance().getConnection();
             PreparedStatement ps = cnx.prepareStatement(buildStrategySelectSql(cnx, resolveStrategyTableName(cnx), "WHERE s.idProj = ? ORDER BY s.idStrategie DESC"))) {
    ...
    public Strategie getStrategieByNom(String nomStrategie) {
        ...
        try (Connection cnx = MyConnection.getInstance().getConnection();
             PreparedStatement ps = cnx.prepareStatement(buildStrategySelectSql(cnx, resolveStrategyTableName(cnx), "WHERE s.nomStrategie = ? LIMIT 1"))) {
...
""".formatted(
    ...
    private Strategie map(ResultSet rs) throws SQLException {
        ...
        // DureeTerme may be absent in older DB schemas.
        try {
    ...
    private String buildStrategySelectSql(Connection cnx, String strategyTable, String tailClause) {
        String st = (strategyTable == null || strategyTable.isBlank()) ? "strategies" : strategyTable;
        String pt = resolveProjectTableName(cnx);
        boolean hasDuree = hasColumn(cnx, "strategies", "DureeTerme");
        if (!hasDuree) {
            hasDuree = hasColumn(cnx, st, "DureeTerme");
        }
        String dureeExpr = hasDuree ? "s.DureeTerme" : "180 AS DureeTerme";
        String titleExpr;
        if (pt == null || pt.isBlank()) {
            titleExpr = "NULL AS titleProj";
        } else {
            titleExpr = "p.titleProj";
        }

        String joinClause = (pt == null || pt.isBlank())
                ? ""
                : "LEFT JOIN " + pt + " p ON p.idProj = s.idProj";
        ...
        return """
            ...
            %s
            %s
        """.formatted(dureeExpr, titleExpr, st, joinClause, safe(tailClause));
    }

    private boolean hasColumn(Connection cnx, String tableName, String columnName) {
        String sql = """
                SELECT 1
    ...
    private String resolveStrategyTableName(Connection cnx) {
        for (String candidate : List.of("strategies", "strategie", "strategy")) {
            if (hasTable(cnx, candidate)) {
                return candidate;
            }
        }
        return "strategies";
    ...
    private String resolveProjectTableName(Connection cnx) {
        for (String candidate : List.of("projects", "Projects", "project")) {
            if (hasTable(cnx, candidate)) {
                return candidate;
            }
        }
    ...
    private boolean hasTable(Connection cnx, String tableName) {
        String sql = """
                SELECT 1
    ...
    private String mapEnumLiteral(String columnType, StrategyStatut status) {
        ...
        for (String v : values) {
            ...
            if (status == StrategyStatut.NON_AFFECTEE && normalized.contains("NON_AFFECTE")) return literal;
        }
        return null;
    }
