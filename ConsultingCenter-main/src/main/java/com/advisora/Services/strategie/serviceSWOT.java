...
public class serviceSWOT {
        ...
        try (Connection c = MyConnection.getInstance().getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    SWOTType parsedType = parseType(rs.getString("type"));
                    if (parsedType == null) {
                        // Skip legacy/corrupted rows instead of crashing chart loading.
                        continue;
                    }
                    it.setType(parsedType);
    ...
    private SWOTType parseType(String raw) {
        if (raw == null) return null;
        String x = raw.trim().toUpperCase();
        if (x.isEmpty()) return null;
        return switch (x) {
            case "STRENGTH", "FORCE", "FORCES" -> SWOTType.STRENGTH;
            case "WEAKNESS", "FAIBLESSE", "FAIBLESSES" -> SWOTType.WEAKNESS;
            case "OPPORTUNITY", "OPPORTUNITIES", "OPPORTUNITE", "OPPORTUNITES" -> SWOTType.OPPORTUNITY;
            case "THREAT", "THREATS", "MENACE", "MENACES" -> SWOTType.THREAT;
            default -> {
                try {
                    yield SWOTType.valueOf(x);
