...
public class ResourceListController {
    ...
    public void initialize() {
        try {
            ...
            com.advisora.utils.i18n.LangBus.localeProperty().addListener((obs, oldL, newL) -> {
    ...
    private void showAdminAiDecisionWindow(AdminAnalysisResult result) {
        ...
        translateFrAsync("Copilote Admin IA - Analyse Decisionnelle", title::setText);
        translateFrAsync(subtitle.getText(), subtitle::setText);
        ...
        translateFrAsync("Filtre priorite", filterLabel::setText);
        ...
        if (table.getPlaceholder() instanceof Label ph) {
            translateFrAsync("Aucune action pour ce filtre.", ph::setText);
        }
        ...
        translateFrAsync(safe(result.getAiSummary()), summaryArea::setText);

        Label summaryTitle = new Label("Synthese IA");
        summaryTitle.getStyleClass().add("module-subtitle");
        translateFrAsync("Synthese IA", summaryTitle::setText);
        ...
        translateFrAsync("Export CSV", btnExportCsv::setText);
        ...
        translateFrAsync("Export PDF", btnExportPdf::setText);
        ...
        translateFrAsync("Fermer", btnClose::setText);
        ...
        Scene scene = SceneThemeApplier.createScene(root, 1260, 780);
    ...
    private TableView<AdminActionItem> buildAdminAiTable() {
        ...
        translateFrAsync("Priorite", colPriority::setText);
        ...
        translateFrAsync("Code Action", colCode::setText);
        ...
        colAction.setCellFactory(col -> new TableCell<>() {
            ...
            protected void updateItem(String item, boolean empty) {
                ...
                setText(item);
                translateFrAsync(item, this::setText);
            }
        });
        translateFrAsync("Action Recommandee", colAction::setText);
        ...
        colIndicator.setCellFactory(col -> new TableCell<>() {
            @Override
            protected void updateItem(String item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
        ...
        translateFrAsync("Indicateur", colIndicator::setText);
        ...
        colImpact.setCellFactory(col -> new TableCell<>() {
            @Override
            protected void updateItem(String item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
        ...
        translateFrAsync("Impact Metier", colImpact::setText);
        ...
        translateFrAsync("Justification", colJustification::setText);
        ...
        translateFrAsync("Confiance", colConfidence::setText);
        ...
        translateFrAsync("Delai", colDeadline::setText);
        ...
        translateFrAsync("Statut", colStatus::setText);
    ...
    private VBox buildAiKpiCard(String title, String value) {
        ...
        translateFrAsync(safe(title), kpiTitle::setText);
    ...
    private VBox buildCard(Ressource r) {
        ...
        translateFrAsync("#" + r.getIdRs() + " - " + safe(r.getNomRs()), title::setText);

        Label badge = new Label(statusDisplayText(r.getAvailabilityStatusRs()));
        badge.getStyleClass().add("status-badge");
        badge.getStyleClass().add(statusClassFor(r.getAvailabilityStatusRs()));
        translateFrAsync(statusDisplayText(r.getAvailabilityStatusRs()), badge::setText);
        Region spacer = new Region();
        HBox.setHgrow(spacer, Priority.ALWAYS);
        ...
        translateFrAsync("Prix: " + r.getPrixRs() + "   |   Quantite: " + r.getQuantiteRs(), meta1::setText);
        Label meta2 = new Label("Fournisseur: " + supplierLabelFor(r.getIdFr()));
        meta2.getStyleClass().add("card-meta");
        translateFrAsync("Fournisseur: " + supplierLabelFor(r.getIdFr()), meta2::setText);
        ...
        translateFrAsync("Projets lies: " + (projectKeywords.isBlank() ? "-" : projectKeywords), meta3::setText);
    ...
    private String statusDisplayText(RessourceStatut status) {
        if (status == RessourceStatut.AVAILABLE) return "Disponible";
        if (status == RessourceStatut.UNAVAILABLE) return "Indisponible";
        if (status == RessourceStatut.RESERVED) return "Reservee";
        return "-";
    }
    ...
    private String targetLang() {
        return com.advisora.utils.AppLanguage.translationTargetCode();
    }

    private void translateFrAsync(String raw, Consumer<String> onUi) {
        String x = safe(raw);
        if (x.isBlank() || "-".equals(x)) {
            onUi.accept(x);
            return;
        }
