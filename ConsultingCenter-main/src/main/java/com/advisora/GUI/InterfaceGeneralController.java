...
public class InterfaceGeneralController {
    ...
    private void loadProjectListIntoContent() throws Exception {
        ...
        loaded.controller.setContentNavigator(this::setContentRoot);
        setContentRoot(loaded.root);
    }

    private void setContentRoot(Parent root) {
        contentHost.getChildren().setAll(root);
        if (contentHost.getScene() != null) {
            try {
                UserPreferencesManager.applySavedToScene(contentHost.getScene());
            } catch (Exception ex) {
                // Never block page navigation because of preferences rendering.
                ex.printStackTrace();
            }
        }
    }
    ...
    private void handleToggleTheme() {
        ...
        try {
            UserPreferencesManager.UserPreferences current = UserPreferencesManager.get();
            UserPreferencesManager.ThemeChoice nextChoice =
                    mode == ThemeMode.DARK
                            ? UserPreferencesManager.ThemeChoice.DARK
                            : UserPreferencesManager.ThemeChoice.LIGHT;
            UserPreferencesManager.saveAndApplyAll(
                    new UserPreferencesManager.UserPreferences(
                            nextChoice,
                            current.fontSizePx(),
                            current.compact(),
                            current.language()
            ...
            // Keep UI responsive even if pref save fails.
        }
    ...
    private void applyProfileImage(User u) {
        if (profileImageView == null) return;
        String rawPath = (u == null) ? null : u.getImagePath();
        profileImageView.setImage(loadUserProfileImage(rawPath));
    }

    private Image loadUserProfileImage(String rawPath) {
        Image fallback = defaultProfileImage();
        ...
        try {
            ...
            if (!candidate.isAbsolute()) {
                String cleaned = (p.startsWith("/") || p.startsWith("\\")) ? p.substring(1) : p;
                candidate = Paths.get(System.getProperty("user.dir")).resolve(cleaned).normalize();
            }
            if (Files.exists(candidate)) {
                return new Image(candidate.toUri().toString());
            }

            Path uploadsCandidate = Paths.get(System.getProperty("user.dir"), "uploads", new File(p).getName());
            if (Files.exists(uploadsCandidate)) {
                return new Image(uploadsCandidate.toUri().toString());
            }
        } catch (Exception ignored) {
        }
        return fallback;
    ...
    private Image defaultProfileImage() {
        var stream = getClass().getResourceAsStream("/GUI/Admin/icons/profile.png");
