...
public class StrategieListController {
    ...
    private boolean statusFilterUserTouched = false;
    private boolean projectFilterUserTouched = false;
    ...
    private String targetLang() {
        return com.advisora.utils.AppLanguage.translationTargetCode(); // "fr" or "en"
    }

    /**
     * Sets label to "prefix + raw", then translates only "raw" and updates label.
     * Example: prefix="Projet associÃ© : ", raw="Mon Projet"
     */
...
///   ///////countdown
    private static LocalDateTime computeDeadlineFromLockedAt(Strategie s) {
        ...
        int days = s.getDureeTerme();     // âœ… int (DB int)
        if (days <= 0) return null;

        LocalDateTime lockedAt = s.getLockedAt(); // âœ… LocalDateTime
        if (lockedAt == null) return null;
    ...
    public void initialize() {
        ...
        com.advisora.utils.i18n.LangBus.localeProperty().addListener((obs, o, n) -> {
            ...
            // âœ… THIS forces updateItem() -> buildCard() -> translations run again
        ...
        strategieList.setCellFactory(lv -> new ListCell<>() {
            ...
            /** One timer per cell â€“ keeps the countdown ticking. */
            ...
            protected void updateItem(Strategie s, boolean empty) {
                ...
                // âœ… 1) stop old timer because ListView cells are reused
                ...
                // âœ… 3) build UI card + get the countdown label directly (no lookup)
                ...
                // âœ… 4) compute deadline using LOCKED_AT (start) + dureeTerme (days)
                ...
                Runnable tick = () -> {
                    ...
                    if (secsLeft <= 0) {
                        countdownLbl.setText("DurÃ©e restante : ExpirÃ©e");
                        ...
                        countdownLbl.setText("DurÃ©e restante : " + formatRemaining(secsLeft));
                        // optional styling class
                ...
                // âœ… 6) start timeline (updates every second)
            ...
            /* -----------------------------------------------------------------------
             * Helpers â€“ keep them in the controller so all other methods can use them
             * ----------------------------------------------------------------------- */
    ...
    public void onFilterStatus(ActionEvent e) {
        statusFilterUserTouched = true;
        applyFilter(txtSearch == null ? "" : txtSearch.getText());
    }

    @FXML
    public void onFilterProject(ActionEvent e) {
        projectFilterUserTouched = true;
        applyFilter(txtSearch == null ? "" : txtSearch.getText());
    }
    ...
    public void onRefresh(ActionEvent e) {
        try {
            ...
            ex.printStackTrace();
            showError("RafraÃ®chissement impossible: " + ex.getMessage());
        }
    }
    ...
    private void resetFilters() {
        if (cmbProjectFilter != null) {
            String allLabel = com.advisora.utils.i18n.I18n.tr("strategie.filter.allProjects");
            if (allLabel == null || allLabel.isBlank()) {
                allLabel = "Tous";
            }
            if (!cmbProjectFilter.getItems().contains(allLabel)) {
                cmbProjectFilter.getItems().add(0, allLabel);
            }
            cmbProjectFilter.setValue(allLabel);
        }
        if (cmbStatusFilter != null) {
            cmbStatusFilter.setValue(null);
            cmbStatusFilter.getSelectionModel().clearSelection();
        ...
        statusFilterUserTouched = false;
        projectFilterUserTouched = false;
    }
    ...
    private void refresh() throws Exception {
        ...
        for (Strategie s : allStrategies) {
            map.put(s.getId(), strategieService.getObjectivesByStrategie(s.getId())); // nÃ©cessite la mÃ©thode
        }
    ...
    private void applyFilter(String q) {
        ...
        StrategyStatut statusFilterRaw = (cmbStatusFilter != null) ? cmbStatusFilter.getValue() : null;
        String projectFilterRaw = (cmbProjectFilter != null) ? cmbProjectFilter.getValue() : null;

        final StrategyStatut finalStatusFilter =
                statusFilterUserTouched ? statusFilterRaw : null;

        final String finalProjectFilter =
                (!projectFilterUserTouched && isAllProjectsValue(projectFilterRaw))
                        ? null
                        : (isAllProjectsValue(projectFilterRaw) ? null : projectFilterRaw);

        List<Strategie> filtered = allObs.stream()
                .filter(s -> !pendingOnly || s.getStatut() == StrategyStatut.EN_ATTENTE)
                .filter(s -> finalStatusFilter == null || s.getStatut() == finalStatusFilter)
    ...
    private CardWithCountdown buildCard(Strategie s) {
        ...
        if (s.getStatut() != null) {
            switch (s.getStatut()) {
                ...
                case EN_ATTENTE -> statut.getStyleClass().add("status-pending");
                case EN_COURS -> statut.getStyleClass().add("status-progress");
                case REFUSEE -> statut.getStyleClass().add("status-refused");
                case NON_AFFECTEE -> statut.getStyleClass().add("status-archived");
                default -> {
                    // keep default badge colors
                }
            }
        }
        ...
        Label gain = new Label("Gain estimÃ© : " + gainText);
        ...
        swotBtn.getStyleClass().addAll("btn-base", "btn-ghost");
        ...
        if (canManage()) {
            ...
            edit.getStyleClass().addAll("btn-base", "btn-ghost");
            edit.setOnAction(e -> openEditStrategieDialog(s));

            Button delete = new Button("Supprimer");
            delete.getStyleClass().addAll("btn-base", "btn-danger-outline");
            delete.setOnAction(e -> deleteStrategie(s));

            Button addObjective = new Button("Attribuer des objectifs");
            addObjective.getStyleClass().addAll("btn-base", "btn-ghost");
        ...
        if (isAdmin() && s.getStatut() == StrategyStatut.EN_ATTENTE) {
            ...
            accept.getStyleClass().addAll("btn-base", "btn-success");
            accept.setOnAction(e -> acceptStrategie(s));

            Button refuse = new Button("Refuser");
            refuse.getStyleClass().addAll("btn-base", "btn-danger");
    ...
    private void openAddDialog() {
        try {
            ...
            if (url == null) {
                showError("FXML introuvable: /views/strategie/AddStrategieh.fxml");
                return;
            }
    ...
    private void openChartsDialog() {
        try {
            ...
            // âœ… click point -> open edit dialog (or info)
    ...
    private void openSwotDialog(Strategie strategie) {
        try {
            ...
            // optionnel: quand on ajoute/modifie, refresh pour mettre Ã  jour "SWOT : x points"
            c.setOnSaved(() -> {
                try { refresh(); } catch (Exception ex) { showError(ex.getMessage()); }
            });
            ...
            Throwable root = ex;
            while (root.getCause() != null) root = root.getCause();
            String details = (root.getMessage() == null || root.getMessage().isBlank())
                    ? ex.toString()
                    : root.getClass().getSimpleName() + ": " + root.getMessage();
            showError("Impossible d'ouvrir SWOT: " + details);
        }
    }

    private void openEditStrategieDialog(Strategie strategie) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/strategie/AddStrategie.fxml"), I18n.bundle());
            Parent content = loader.load();
    ...
    private void openAddObjectiveDialog(Strategie strategie) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/objectif/AddObjectif.fxml"), I18n.bundle());
            Parent content = loader.load();
    ...
    private void openObjectiveInfoDialog(Objective objective, Strategie strategie) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/objectif/ObjectiveInfoDialog.fxml"), I18n.bundle());
            Parent content = loader.load();
    ...
    private void refuseStrategie(Strategie s) {
        ...
        dialog.setHeaderText("Refuser cette stratÃ©gie");
    ...
    private boolean isAllProjectsValue(String value) {
        if (value == null || value.isBlank()) return true;
        String v = value.trim();
        String i18nAll = com.advisora.utils.i18n.I18n.tr("strategie.filter.allProjects");
        return v.equalsIgnoreCase("Tous")
                || v.equalsIgnoreCase("All")
                || (i18nAll != null && v.equalsIgnoreCase(i18nAll.trim()));
    }

    // âœ… THIS is the countdown core used by the cell:
    private static LocalDateTime computeDeadline(Strategie s) {
        return computeDeadlineFromLockedAt(s);
    }

    private void ensureStylesheets(Parent root) {
        if (root == null) return;
        addStylesheetIfMissing(root, "/css/base.css");
        addStylesheetIfMissing(root, "/css/components.css");
        addStylesheetIfMissing(root, "/css/strategie-list.css");
    }

    private void addStylesheetIfMissing(Parent root, String path) {
        try {
